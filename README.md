...
适合独立开发者、内容创作者和高频 AI 用户的 Windows 语音输入工具：按住全局热键说话，松开后自动识别、润色、翻译并输入到当前窗口。
...
- 🤖 **AI 优化润色**：可选 OpenAI 兼容 API 做润色、翻译、重写（中文说话 → 英文输出，也可转换为中国香港繁体 / 中国台湾繁体）
...
口语化输入可以自动整理为更容易理解、更适合直接发送给 AI 或他人的文本。

![SpeakIn 默认文本润色示例](./docs/images/speakin-polish.png)

### 口语转英文

通过 AI 优化提示词，可以把中文口语输入自动转换为英文，也可以按需要输出为中国香港繁体、中国台湾繁体等不同文本风格。

![SpeakIn 口语转英文示例](./docs/images/speakin-translate-en.png)

## 安装

### Windows（推荐）

前往 [Releases](https://github.com/magiccodelab/speakIn/releases) 页面下载最新的 `.exe` 安装包。
...
SpeakIn 目前支持三家云端 ASR，任选其一（均有免费额度）：

| 供应商 | 文档 | 免费额度 |
|---|---|---|
| **豆包** (火山引擎) | [控制台](https://console.volcengine.com/speech/service/10011) | 每个应用 20 小时免费额度 |
| **百炼** (DashScope) | [控制台](https://bailian.console.aliyun.com/) | 有免费额度 |
| **千问 Realtime** | [控制台](https://bailian.console.aliyun.com/) | 有免费额度 |

说明：
- 百炼和千问 Realtime 同属阿里云百炼体系，可使用同一个国内站 API Key。
- 阿里云国内站与国际站的 API Key 不互通。国内站 API Key 无法调用国际站 ASR 服务，国际站 API Key 也无法用于国内站服务。
- 豆包 ASR 的免费额度按应用发放。单个应用免费额度用完后，可在火山引擎控制台新建应用并使用新应用的凭据继续享受对应免费额度。
...
- [SpeakIn 完整使用指南，从零配置豆包语音识别到高阶优化](https://afengblog.com/blog/speakin-settings-guide.html)：从安装、ASR 供应商配置到 API Key 等基础设置，都可以参考这篇教程。
- [怎么给 SpeakIn 写一条 AI 优化提示词](https://afengblog.com/blog/speakin-prompt.html)：如果想自定义润色、翻译、繁简转换等输出效果，可以参考这篇提示词写法说明。
- [默认提示词页面](https://afengblog.com/prompts)：如果想直接获取默认提示词或参考已有提示词模板，可以访问这个页面。
